...
class MailThreadBlacklist(models.AbstractModel):
    ...
    A normalized email is considered as:
        ...
        - having a left part + @ + a right part (domain without '.something' is ok)
        - being lower case
        - having no name before the address. Typically no 'Name <>'
    ...
    The primary email field can be specified on the parent model if it differs from
    the default one ('email'). The email_normalized field can then be used to search
    quickly on emails (simple comparison, not regex).

    Using this field, blacklist status is computed.

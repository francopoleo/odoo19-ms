# Troubleshooting — Documento Fiscal Provisório LATAM/Brasil
...
```bash
./odoo-bin -d ms-teste -u account_latam_provisional_post,common_help_center --stop-after-init
```
...
```sql
DELETE FROM help_article WHERE module_name = 'account_latam_provisional_post';
DELETE FROM help_context WHERE module_name = 'account_latam_provisional_post';
```

# Troubleshooting — Payment Provider: PIX (BACEN)
...
```sql
DELETE FROM help_article WHERE module_name = 'payment_pix';
DELETE FROM help_context WHERE module_name = 'payment_pix';
```

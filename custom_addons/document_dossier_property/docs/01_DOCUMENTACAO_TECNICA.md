# Documentação Técnica — Document Dossier - Property Integration
...
Liga dossiês documentais a imóveis/contratos para organizar documentos formais.

## 2. Manifesto e dependências

| Item | Valor |
|---|---|
| Módulo técnico | `document_dossier_property` |
| Nome funcional | Document Dossier - Property Integration |
| Versão | `19.0.2.1.2` |
| Aplicação | `False` |
| Instalável | `True` |
| Dependências | `document_dossier`, `property_core` |

### Arquivos declarados no manifesto

- `views/property_asset_ext_views.xml`
- `views/property_contract_ext_views.xml`
...
- **Classe:** `PropertyAssetExt`
- **Arquivo:** `models/property_asset_ext.py`
...
| `action_create_purchase_dossier` | Ação de botão/fluxo | Validar permissões, mensagens, efeitos no registro, atividades e Agenda Geral. |
| `action_create_sale_dossier` | Ação de botão/fluxo | Validar permissões, mensagens, efeitos no registro, atividades e Agenda Geral. |
| `action_create_lease_dossier` | Ação de botão/fluxo | Validar permissões, mensagens, efeitos no registro, atividades e Agenda Geral. |

### Model `property.contract`

- **Classe:** `PropertyContractExt`
- **Arquivo:** `models/property_contract_ext.py`
- **Descrição técnica:** não declarada no código; manter esta descrição atualizada quando o model evoluir.
...
|---|---|---|---|---|
| `property.asset` | `models/property_asset_ext.py` | Modelo `property.asset` usado pelo módulo. | dossier_ids, dossier_count | _compute_dossier_count, action_assign_dossier_template, action_view_dossiers, action_create_purchase_dossier, action_create_sale_dossier, action_create_lease_dossier |
| `property.contract` | `models/property_contract_ext.py` | Modelo `property.contract` usado pelo módulo. | dossier_id, dossier_completion_percent, dossier_complete, dossier_document_count, dossier_missing_requirements | action_assign_dossier_template, action_view_dossier, action_view_dossier_documents |
...
|---|---|---|
| `view_property_asset_form_dossier_ext` | `property.asset` | `views/property_asset_ext_views.xml` |
| `view_property_asset_list_dossier_ext` | `property.asset` | `views/property_asset_ext_views.xml` |
| `view_property_contract_form_dossier_ext` | `property.contract` | `views/property_contract_ext_views.xml` |
| `view_property_contract_list_dossier_ext` | `property.contract` | `views/property_contract_ext_views.xml` |

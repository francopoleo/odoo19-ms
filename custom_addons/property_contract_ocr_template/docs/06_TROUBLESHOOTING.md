# Troubleshooting — Property Contract OCR Templates
...
```bash
./odoo-bin -d ms-teste -u property_contract_ocr_template,common_help_center --stop-after-init
```
...
```sql
DELETE FROM help_article WHERE module_name = 'property_contract_ocr_template';
DELETE FROM help_context WHERE module_name = 'property_contract_ocr_template';
```

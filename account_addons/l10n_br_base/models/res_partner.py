...
class Partner(models.Model):
    ...
    vat_display = fields.Char(
        string="CPF/CNPJ",
        compute="_compute_vat_display",
        help="Formatted VAT for display only",
    )
    ...
    @api.depends("vat")
    def _compute_vat_display(self):
        """Format VAT/CNPJ/CPF for display only."""
        for record in self:
            if record.vat:
                record.vat_display = cnpj_cpf.formata(record.vat)
            else:
                record.vat_display = ""
